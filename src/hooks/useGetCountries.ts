...
const filterByNameAndRegion = (
  country: CountryBasicInfo,
  filters: ContriesFilters
) =>
...
const useGetCountries = (filters: ContriesFilters) => {
  return useQuery(`countries`, getCountries, {
    select: countries =>
      countries
        .sort(sortByName)
        .filter(country => filterByNameAndRegion(country, filters))
  });
};
...
const useGetCountriesByCca2 = (cca2: string[]) => {
  return useQuery(
    ['countriesByCca2', cca2.join(',')],
    () => getCountriesByCca2(cca2),
    {
